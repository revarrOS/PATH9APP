...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const orchestratePayload = {
      user_id: user.id,
      request_id: `condition_${Date.now()}`,
    ...
    console.error("Error in condition-ai-respond:", error);
    return new Response(
...
function detectConsultationPrepOpportunity(
  ...
  return {
    suggestedQuestion: questionMatch?.[1] || "Can you help me understand what's changed in my condition?",
    relatedTerms: [],
    sourceContext: {},
  };
}
