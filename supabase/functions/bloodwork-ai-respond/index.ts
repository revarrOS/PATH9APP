...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const orchestratePayload = {
      user_id: user.id,
      request_id: `bloodwork_${Date.now()}`,
    ...
    return new Response(
      JSON.stringify({
        ...
        ...(consultationPrepSuggestion && {
          ...
          relatedMarkers: consultationPrepSuggestion.relatedMarkers,
    ...
    console.error("Error in bloodwork-ai-respond:", error);
    return new Response(
...
function detectConsultationPrepOpportunity(
  ...
  const markerPatterns = /\b(WBC|RBC|HGB|HCT|PLT|NEUT|LYMPH|MONO|platelet|white blood cell|hemoglobin|haemoglobin)\b/gi;
  const mentionedMarkers = [...new Set([
    ...(userMessage.match(markerPatterns) || []),
    ...(aiResponse.match(markerPatterns) || [])
  ])];

  return {
    suggestedQuestion: questionMatch?.[1] || "Can you help me understand what's happening with my blood work?",
    relatedMarkers: mentionedMarkers.slice(0, 3),
    sourceContext: {},
  };
}
