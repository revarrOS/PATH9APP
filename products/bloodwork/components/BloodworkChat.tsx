...
import { consultationPrepStore } from '@/products/bloodwork/consultation-prep/services/consultation-prep.store';
...
export function BloodworkChat() {
  ...
  const checkGemmaAvailability = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.EXPO_PUBLIC_SUPABASE_URL}/functions/v1/bloodwork-ai-respond`,
        {
          method: 'POST',
          headers: {
  ...
  const handleSaveQuestion = async (questionText: string, relatedMarkers: string[]) => {
    if (!selectedSuggestion) return;

    try {
      await consultationPrepStore.addQuestion(questionText, {
        relatedMarkers,
        source: 'gemma',
  ...
  const handleSend = async () => {
    ...
    try {
      await gemmaConversationService.addMessage(userMessage, 'bloodwork');
      ...
      const domainContext = gemmaConversationService.getDomainContextPrompt('bloodwork');
      ...
      const assistantMessage: Message = {
        ...
        consultationPrepSuggestion: data.showSaveToConsultationPrep
          ? {
              ...
              relatedMarkers: data.relatedMarkers || [],
              sourceContext: data.sourceContext,
            }
          : undefined,
      };

      await gemmaConversationService.addMessage(assistantMessage, 'bloodwork');
    ...
    <View style={styles.container}>
        ...
        {conversation.length === 0 && (
          <View style={styles.welcomeContainer}>
            <Text style={styles.welcomeText}>
              I can help you understand your bloodwork numbers and prepare questions for your clinician.
            </Text>
            <Text style={styles.welcomeSubtext}>
              Ask me about your blood test results, trends over time, or what specific markers generally represent.
            </Text>
          </View>
        )}
      ...
      <View style={styles.inputContainer}>
        <TextInput
          ...
          placeholder="Ask about your bloodwork..."
        ...
        <View style={styles.continuationModalOverlay}>
          <View style={styles.continuationModal}>
            ...
            <Text style={styles.continuationDescription}>
              Keep exploring your bloodwork insights without interruption.
            </Text>
